...
You can also get the [package.json file](../package.json) because it has scripts setup up for quick server administration. You'll need `npm` available for that, and this is only because of the `scripts` section in it, so running `npm install` is entirely unneccessary.
