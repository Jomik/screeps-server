...
From the folder that contains `docker-compose.yml` (your project root), use `npm` or the `database.sh` script. Backups themselves are written to a `backups` folder next to your compose file. It's recommended to `.gitignore` that directory if you're using git.
...
or alternatively
```sh
./scripts/database.sh backup before-upgrade
```
...
The script pauses the server, backs up Mongo and Redis, then resumes. This means players only see a paused world during the backup window instead of a hard disconnect from stopping the container.
...
With npm you need to add a `--` so the prefix is passed to the script correctly:
...
```sh
./scripts/database.sh restore 20260113-120000
```

You may pass something like `backups/20260113-120000` or a full filename; the script figures out the prefix.
